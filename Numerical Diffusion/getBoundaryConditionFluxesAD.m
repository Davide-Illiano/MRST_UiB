...
% Basic quanitites
T = model.operators.T_all(bc.face) .* 4.96*10^-2;
G = model.G;
...
%         %Top and bottomo contribution
%         yTop = max(G.nodes.coords(:,2));
%         ez_val = [yTop * ones((G.cartDims(1)),1)' zeros((G.cartDims(1)),1)']';
%         pBC   = cellToBCMap*pressure{i};
%         ezBC = cellToBCMap*ez ;
%         ez_contribution = ez_val - ezBC;
%         
%         dP = bc.value(isP) - pBC(isP) - ez_contribution;
