function [src, bc, dC, ez_contribution] = computeSourcesAndBoundaryConditionsandConcAD(model, pressure,c, s, mob, rho, dissolved, forces)
...
%}
    ...
    if hasBC || hasSRC
        ...
        if hasBC
            ...
            % getBoundaryConditionFluxesAD(model, pressure,  s, mob, rho, b, forces.bc);
            %[qSurf, BCTocellMap, BCcells, qRes, dC], ez_contribution
            [qVolBC, BCTocellMap, bcCells, qRes, dC] = getBoundaryConditionFluxesandConcAD(model, pressure, c, s, mob, rho, b, forces.bc);
        end
