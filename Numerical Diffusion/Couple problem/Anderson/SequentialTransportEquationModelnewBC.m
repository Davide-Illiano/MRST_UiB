    ...
    methods
       ...
       function [eqs, state, src] = addBoundaryConditionsAndSources(model, eqs, names, types, state, ...
        ...
        for i = 1:numel(cnames)
            ...
            [eq, bnd_cond] = model.addComponentContributions(name, eq, C, bnd_cond, forces.bc, dC);
        ...
        if nargout > 2
            ...
            function [eq, src] = addComponentContributions(model, cname, eq, component, src, force, dC)
       ...
       if isempty(force)
        ...
        switch lower(cname)
            case {'concentration'}
                ...
                pv = max(poreVolume(model.G,model.rock));
                qC = qC + 1e-3.*dC; 
            otherwise
