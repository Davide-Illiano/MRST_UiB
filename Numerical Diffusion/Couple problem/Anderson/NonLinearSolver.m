classdef NonLinearSolver < handle
    ...
    properties
        ...
        continueOnFailure % Continue evƒen if failure is reported by the model. Results are most likely not useful. Intended for nested nonlinear solvers.
    end
    ...
    methods
        function solver = NonLinearSolver(varargin)
            disp('Using the classical sequential approach, multiple iterations on each equation');
            solver.maxIterations   = 1000; %25
            solver.minIterations   = 1;
            solver.identifier      = '';
            solver.verbose         = mrstVerbose();
            solver.maxTimestepCuts = 6; %6 
            solver.LinearSolver    = [];
            ...
            solver.linesearchMaxIterations = 10; %10
            ...
            solver.stagnateTol = 1e-2; %-2
            solver.convergenceIssues = true; %true

            solver.errorOnFailure = false;  %true
            solver.continueOnFailure = true; %false
        ...
        function [state, report, ministates] = solveTimestep(solver, state0, dT, model, varargin)
            ...
            %% If model model.stepFunctionIsLinear = true we force only one teration
            model.stepFunctionIsLinear = true; %false
        ...
        function [state, failure, report] = solveMinistep(solver, model, state, state0, dt, drivingForces)
            ...
            for i = 1:(maxIts + 1)
                ...
                if converged   % when the inside loop comverge we break the loop,
                               %try to add the anderson accelereation here
                               
                    state.pressureUpd = state.pressure;
                    state.cUpd = state.c;
                    
                    % determine how many iterations we had
       ...
       if 1 == 1 
           m = 2;
                m_i = min(i,m);
                % Save the report of each iteration
                % AndersonReports(i) = stepReport;   
                % Need residuals of iterations stepReport.Residuals
                % minimisazion problem min_a ||a*residuals|| s.t. norm a=1
                % new updated solution is x_{k+1} = sum_k a_k * g(x_k) .  g is
                % matrix A = stepReport.A
                
                % Minimization problem can be solved with function fmincon
                % first define function f
                a0 = 1/m_i * ones(1,m_i); % initial guess for a
                beta = 1;
                
               %% if altLin
          if   model.stepFunctionIsLinear  == 1 
                    res_p(:,m_i) = stepReport.PressureSolver.StepReports{1}.NonlinearReport{1}.LinearSolver.VectorResiduals.pressure;
                    res_c(:,m_i) = cell2mat(stepReport.TransportSolver.StepReports{1}.NonlinearReport{1}.LinearSolver.VectorResiduals.c);
                    ...
                    if m_i == 1
                          ...
                          a_c = 1;
                            ...
                            A_p = getF(res_p);  % work on it [r_1 - r_n, r_2 - r_n, ... , r_n-1 - r_n]
                            b_p = - res_p(:,size(res_p,2)); % ok
                            [Q, R] = qr(A_p,0); % take reduce one, only same colums as A
                            a_p = R^-1 * Q' * b_p;
                            a_p(m_i) = 1 - sum(a_p);
                            if size(a_p,1) > size(a_p,2)
                                a_p = a_p';
                            end
                    
                            
                            A_c = getF(res_c);
                            b_c = -res_c(:,size(res_c,2));
                            [Q, R] = qr(A_c,0);
                            a_c = R^-1 * Q' * b_c;
                            a_c(m_i) = 1 - sum(a_c);
                            if size(a_c,1) > size(a_c,2)
                                a_c = a_c';
                            end
                    end
                            
                    pressure(:,m_i) = state.pressure;
                    c(:,m_i) = state.c;
                    
                    pressure_sum = beta * sum(a_p.*pressure,2) ;
                    c_sum = beta * sum(a_c.*c,2) ;
                
                    state.pressure = pressure_sum;
                    state.c = c_sum;
              ...
              %% NonLinS
              beta = 1; 
              if size(stepReport.Residuals,2) == 2
              else
               
               if isempty(stepReport.LinearSolver) == 1
               else
               if isfield(stepReport.LinearSolver.VectorResiduals, 'pressure') == 1
                   res_p(:,m_i) = stepReport.LinearSolver.VectorResiduals.pressure;
                   ...
                   if isempty(stepReport.LinearSolver)  
                   else
                   res_c(:,m_i) = cell2mat(stepReport.LinearSolver.VectorResiduals.c);
