...
methods
    ...
    function [state, report] = stepFunction(model, state, state0, dt, drivingForces, linsolver, nonlinsolver, iteration, varargin)
        ...
        % add exetrnal forces
        %{
        problem.equations{1} = problem.equations{1} - drivingForces.analyticalForce1 * dt;
        problem.equations{2} = problem.equations{2} - drivingForces.analyticalForce2 * dt;
        %}        
%% external forces sequential
        %{
        if size(problem.equationNames{1}) == [1 5]   %water has 5 letters
            problem.equations{1} = problem.equations{1} - drivingForces.analyticalForce1 * dt;
        else
            problem.equations{1} = problem.equations{1} - drivingForces.analyticalForce2 * dt;
        end
        %}
...
%                 if model.UseNewApp ==1 
%                     state.p_prev = state.pressure;
%                     state.c_prev = state.c;
%                 end
