function [qSurf, BCTocellMap, BCcells, qRes, dC, ez_contribution] = getBoundaryConditionFluxesandConcAD(model, pressure, c, s, mob, rho, b, bc, vW)
...
%    Treat pressure BC
%         ez =  G.cells.centroids(:,2);
%         ez_val = [3 * ones((G.cartDims(1)),1)' zeros((G.cartDims(1)),1)']';
%         pBC   = cellToBCMap*pressure{i} + cellToBCMap*ez ;
%         dP = bc.value(isRF) + ez_val - pBC(isRF);
%      ez_contribution =  ez_val - cellToBCMap*ez;
     dP = bc.value(isP) - pBC(isP) - dzbc(isP) ;
    % add gradient of concentration
%     dC = bc.c(isP) - cBC(isP);
    %% if sequential/splitting
    dC = bc.c(isRF) - cBC(isRF);
