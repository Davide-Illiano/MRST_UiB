classdef LinearSolverAD < handle
    ...
    methods
        ...
        function [dx, result, report] = solveLinearProblem(solver, problem, model)
            ...
            residual = [];
            if size(problem.equationNames{1},2) == 5
            residual.pressure = dx(1);
            report.VectorResiduals.pressure = residual.pressure{1};
            else
            residual.c = dx(1);
            report.VectorResiduals.c = residual.c;
            end
        ...
        function [A, b, sys] = reduceLinearSystem(solver, A, b, isAdjoint)
            ...
            % Perform Sresidualchur complement reduction of linear system
            sys = struct('B', [], 'C', [], 'D',   [], 'E',   [],...
                         'f', [], 'h', [], 'E_L', [], 'E_U', []);
