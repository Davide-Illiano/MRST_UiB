classdef NonLinearSolver < handle
    ...
    methods
        function solver = NonLinearSolver(varargin)
            ...
            solver.convergenceIssues = false; %true
        ...
        function [state, report, ministates] = solveTimestep(solver, state0, dT, model, varargin)
            ...
            model.stepFunctionIsLinear = false; %false
    ...
    methods
        ...
        function [state, report, ministates] = solveTimestep(solver, state0, dT, model, varargin)
            ...
            while ~done
                ...
                itCount = itCount + its;
                stepCount = stepCount + 1;
            % try to comment out part regarding if convergend. We want to
            % proceed anyway
            % if converged 
                    t_local = t_local + dt;
                    dt_prev = dt;
                ...
                %}   
                done = isFinalMinistep; %isFinalMinistep && converged;
            end
        ...
        function [state, failure, report] = solveMinistep(solver, model, state, state0, dt, drivingForces)
            ...
            for i = 1:(maxIts + 1)
                ...
                [state, stepReport] = ...
                    ...
                    solver.LinearSolver, solver, ...
                    i);
                converged  = 1; %stepReport.Converged;
                failure    = stepReport.Failure;
                reports{i} = stepReport;
                %if converged
                    break
