...
% diffusion/dispersion coeff
    ...
    Lx=I-2; Ly=J-2;
    x0 = 0.3;%round(Lx*dx/8);  %x0=round(Lx*dx/10);
...
%         model.diffusion = 1;
        ...
        numb
%        save(['NewPartial_SplittingTentative2110_RandK_MRST_Mesh(',num2str(dx),',',num2str(dy),')_n(',num2str(n),')_7_num',num2str(numb+5),'.mat'], 'dx', 'Pe', 'Mx', 'My','Varx', 'Vary', 'time_flow', 'time_transp');
    time_realiz = toc
    end
    %%
    %     save(['varK_GAUSS_10_MRST_Mesh(',num2str(I),',',num2str(J),')_',num2str(numb),'.mat'],'c','Vx','Vy') ;
