...
%}
    ...
    if hasBC || hasSRC || hasGs
        ...
        %{
        if model.useRobinG 
            % Setup the Robin boundary condition
            [qVolBC, BCTocellMap, bcCells, qResBC] = getBoundaryRobinConditionAD(model, pressure, forces.bc);
        end
        %}
        if hasSRC %&& ~model.useRobinG
