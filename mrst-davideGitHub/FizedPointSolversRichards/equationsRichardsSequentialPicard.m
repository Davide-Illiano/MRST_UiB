function [problem, state] = equationsRichardsSequentialPicard(state0, state, model, dt, drivingForces, varargin)
...
Kmult = fluid.Kmult(p_prev, sW);    %(p, sW);
...
water = (V./dt).*( bW.*sW - bW0.*sW0 + theta_first_p.*(p - p_prev)) + s.Div(bWvW); 
