function [problem, state] = equationsTransportSequentialPicard(state0, state, model, dt, drivingForces, varargin)
...
theta_first_c = full(diag(theta_ad.jac{1}));  % only c is ad varible for this 
                                              %      reason jac{1} is
                                              %      actually derivative
                                              %      with resect to c
...
transport = (V./dt).*(( bW.*sw - bW0.*sw0 + theta_first_c.*(c - c_prev)).*c_prev + sW.*(c - c0)) ...
    + s.Div(Vc);% -c./(c_prev +1);
