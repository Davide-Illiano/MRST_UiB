...
water = (V./dt).*( theta(p_prev,c_prev) - theta(p0,c0) + model.L_p.*(p - p_prev)) + s.Div(bWvW);

% Transport equation
transport = (V./dt).* (theta(p_prev,c_prev).*c - theta(p0,c0).* c0 + model.L_c.*(c - c_prev)) + s.Div(Vc)  ;
