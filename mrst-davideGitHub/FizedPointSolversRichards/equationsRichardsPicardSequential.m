function [problem, state] = equationsRichardsPicardSequential(state0, state, model, dt, drivingForces, varargin)
...
theta_ad = fluid.theta(p);
theta_first = full(diag(theta_ad.jac{1}));
...
water = (V./dt).*( bW.*theta(p,c) - bW0.*theta(p0,c0) + theta_first.*(p - p_prev)) + s.Div(bWvW); %(V./dt).*( bW.*theta(p) - bW0.*theta(p0) ) + s.Div(bWvW); %
...
% Apply scaling
eqs{1} = eqs{1}.*(dt./V);   % i have removed the scaling
