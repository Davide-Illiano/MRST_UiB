function [problem, state] = equationsOilWaterLScheme(state0, state, model, dt, drivingForces, varargin)
...
[p_prev, sW_prev, wellSol] = model.getProps(state, 'pressure', 'water', 'wellsol');
...
[krW, krO] = model.evaluateRelPerm({sW_prev, sO_prev});

% Multipliers for properties
[pvMult, transMult, mobMult, pvMult0] = getMultipliers(model.fluid, p_prev, p0);
...
[vW, bW, mobW, rhoW, pW, upcw] = getFluxAndPropsWater_BO(model, p_prev, sW_prev, krW, T, gdz);
bW0 = model.fluid.bW(p0);

% Evaluate oil properties
[vO, bO, mobO, rhoO, p, upco] = getFluxAndPropsOil_BO(model, p_prev, sO_prev, krO, T, gdz);
...
water = (s.pv/dt).*( pvMult.*bW.*sW - pvMult0.*bW0.*sW0 + .1 .*(sW - sW_prev));

% Conservation of mass for oil
oil = (s.pv/dt).*( pvMult.*bO.*sO - pvMult0.*bO0.*sO0  + .1 .*(sO - sO_prev));
