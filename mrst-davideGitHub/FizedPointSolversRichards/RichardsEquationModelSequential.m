classdef RichardsEquationModelSequential < ReservoirModel
    properties
        forces
        polymer
        Newton
    ...
    methods
        function model = RichardsEquationModelSequential(G, rock, fluid, varargin)
        ...
        function [problem, state] = getEquations(model, state0, state, dt, drivingForces, varargin)
           ...
           if model.Newton
            [problem, state] = equationsRichardsSequential(state0, state, model,...
                            dt, ...
                            drivingForces,...
                            varargin{:});
           end
           ...
           if model.Picard
            [problem, state] = equationsRichardsSequentialPicard(state0, state, model,...
                            dt, ...
                            drivingForces,...
                            varargin{:});
           end
           ...
           if model.L_p>0
                [problem, state] = equationsRichardsSequentialLScheme(state0, state, model,...
                            dt, ...
                            drivingForces,...
                            varargin{:});
       ...
       function [compEqs, compSrc, eqNames, wellSol] = getExtraWellContributions(model, well, wellSol0, wellSol, q_s, bh, packed, qMass, qVol, dt, iteration)
            ...
            function [eq, src, new_vW] = addComponentContributions(model, cname, eq, component, src, force)
        if isempty(force)
            return
        end
